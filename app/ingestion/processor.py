...
def save_processed_locally(data: dict, source_type: str, filename: str) -> str:
    ...
    # 1. Build directory path, e.g. "processed_data/local_upload"
    folder = os.path.join(PROCESSED_DATA_DIR, source_type)
    # 2. Ensure folder exists (creates parent directories if needed without erroring if it already exists)
    os.makedirs(folder, exist_ok=True)
    # 3. Construct destination file path (e.g. processed_data/local_upload/annual_report_2023.pdf.json)
    dest = os.path.join(folder, f"{filename}.json")
    # 4. Write the processed data to the JSON file with UTF-8 encoding to handle special characters
...
#  processes every file in a directory
