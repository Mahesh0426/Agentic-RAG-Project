...
def retrieve_node(state: AgentState):
    ...
    with logfire.span("🔍 Knowledge Retrieval"):
        ...
        # Retrieve initial candidates using vector similarity search
        raw_results = search_enterprise_knowledge(query, limit=15)
        logfire.info(f"Retrieved {len(raw_results)} candidates from Vector DB")
        
        # Extracts only the text content from the search results by removing {score} and {source}
        ...
        # Formats the final documents by adding "CONTENT: " before each document
